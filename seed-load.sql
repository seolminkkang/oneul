...
SELECT
    ...
    1000
FROM (SELECT 1 AS i UNION ALL SELECT 2 UNION ALL SELECT 3) t;
...
-- ---------- 2-C. 배경 데이터용 진행중 챌린지 29개 (2026-08-13 추가) ----------
-- ★ 왜 필요한가
--   앱에 진입하면 /challenges, /challenges/community, /challenges/subscribed,
--   /feeds/community, /feeds/my, /feeds/streak 가 한꺼번에 나간다(실측).
--   이 테이블들이 비어 있으면 조회가 공짜라 웹 p95 가 비현실적으로 낮게 나온다.
--   2026-08-12 측정이 정확히 그 상태였다.
--
-- 규모 근거: refund-batch-design.md "동시 운영 챌린지 30개"
--   load-challenge-live(1) + 여기 29개 = 30개
INSERT INTO challenge
    (name, owner_id, category_id, description, total_day, goal_day,
     is_challenge, is_public, start_date, end_date, entry_fee, challenge_status, member_count)
SELECT
    CONCAT('load-challenge-live-', i),
    1, 1, '부하 측정용 배경 챌린지',
    30, 20, TRUE, TRUE,
    DATE_SUB(CURDATE(), INTERVAL 10 DAY),
    ...
    10000, 'IN_PROGRESS', 1000
FROM (
    WITH RECURSIVE c AS (SELECT 2 AS i UNION ALL SELECT i + 1 FROM c WHERE i < 30)
    SELECT i FROM c
) t;
...
-- ---------- 5. 배경 데이터: 참가 · 인증 피드 · 스트릭 (2026-08-13 추가) ----------
-- 사용자 1,000명이 배경 챌린지 2개에 추가 참가한다.
--   → live 1개 + 배경 2개 = 1인당 3개 참가
--   근거: "사용자는 여러 챌린지에 동시 참가 가능"(CLAUDE.md 도메인 사실).
--   ★ 단 "평균 3개"라는 값 자체는 유도된 게 아니라 잡은 값이다.

INSERT INTO challenge_user (challenge_id, user_id, success_day, is_refunded, refund_amount)
SELECT c.challenge_id, u.user_id, 8, FALSE, 0
FROM challenge c
JOIN `user` u ON u.oauth_provider = 'LOAD'
WHERE c.name IN ('load-challenge-live-2', 'load-challenge-live-3');

-- 인증 피드: 참가 3,000건 × 최근 15일 = 45,000건
-- like_count 는 RAND() 대신 결정적 계산을 쓴다 (몇 번을 돌려도 같은 상태여야 하므로)
INSERT INTO feed (user_id, challenge_id, image_url, content, like_count, check_status, created_at)
SELECT
    cu.user_id,
    cu.challenge_id,
    -- ★ 반드시 'uploads/' 로 시작해야 한다.
    --   FeedServiceImpl.getFeeds() 가 이 접두사일 때만 S3 presigned URL 을 생성한다.
    --   외부 URL(picsum 등)을 쓰면 그 경로가 통째로 건너뛰어져서
    --   응답 시간이 23.6초 -> 0.36초로 보인다. 측정이 왜곡된다.
    CONCAT('uploads/', cu.id, '-', d.i, '.jpg'),
    CONCAT('부하 배경 인증 ', d.i),
    (cu.id * 7 + d.i * 13) % 50,
    'APPROVED',
    DATE_SUB(NOW(), INTERVAL d.i DAY)
FROM challenge_user cu
JOIN challenge c ON c.challenge_id = cu.challenge_id
CROSS JOIN (
    WITH RECURSIVE d AS (SELECT 1 AS i UNION ALL SELECT i + 1 FROM d WHERE i < 15)
    SELECT i FROM d
) d
WHERE c.name LIKE 'load-challenge-live%';

-- 스트릭: 사용자 1,000명 × 15일
INSERT INTO streak (user_id, `date`)
SELECT u.user_id, DATE_SUB(CURDATE(), INTERVAL d.i DAY)
FROM `user` u
CROSS JOIN (
    WITH RECURSIVE d AS (SELECT 1 AS i UNION ALL SELECT i + 1 FROM d WHERE i < 15)
    SELECT i FROM d
...
-- ---------- 확인 ----------
SELECT '끝난 챌린지'        AS 항목, COUNT(*) AS 값 FROM challenge WHERE name LIKE 'load-challenge-ended-%'
UNION ALL
...
SELECT '진행중 참가자',     COUNT(*) FROM challenge_user cu
    ...
    WHERE c.name = 'load-challenge-live'
UNION ALL
SELECT '진행중 챌린지(30 목표)', COUNT(*) FROM challenge WHERE name LIKE 'load-challenge-live%'
UNION ALL
SELECT '★ 배경 피드(45000 목표)', COUNT(*) FROM feed f
    JOIN challenge c ON c.challenge_id = f.challenge_id
    WHERE c.name LIKE 'load-challenge-live%'
UNION ALL
SELECT '스트릭',           COUNT(*) FROM streak s
    JOIN `user` u ON u.user_id = s.user_id WHERE u.oauth_provider = 'LOAD';
